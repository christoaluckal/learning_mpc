...
class MPC:
	...
	def cost(self, u_k, car, goal_x):
		...
		for i in range(self.horizon):
			controller_car.update(u_k[0,i], u_k[1,i],DELTA_T,from_model=True)
	...
	def optimize(self, car, goal_x):
		...
		init = np.tile([0.2,0.0],self.horizon)
		result = minimize(self.cost, args=(car, goal_x), x0 = init, method='SLSQP', bounds = bnd)
